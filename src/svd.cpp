...
void matrix<T>::svd(matrix<T>& u, matrix<T>& s, matrix<T>& v){
    ...
    if constexpr (std::is_same_v<T, float>){
        ...
        info = LAPACKE_sgesdd (LAPACK_ROW_MAJOR , 'S' , m, n , A_data , lda, s_vec.data(), u_vec.data(), ldu, v_vec.data(), ldv);
        s = matrix<float>(s_vec, m, n);
    }
    else if constexpr (std::is_same_v<T, double>){
        std::vector<double> s_vec (n,0);
        info = LAPACKE_dgesdd (LAPACK_ROW_MAJOR , 'S' , m, n ,A_data, lda, s_vec.data(), u_vec.data(), ldu, v_vec.data(), ldv);
        s = matrix<double>(s_vec, m, n);
    }
    else if constexpr (std::is_same_v<T, std::complex<float>>){
        std::vector<float> s_vec (n,0);
        info = LAPACKE_cgesdd (LAPACK_ROW_MAJOR , 'S' , m, n , reinterpret_cast<MKL_Complex8*> (A_data) , lda, s_vec.data(), reinterpret_cast<MKL_Complex8*> (u_vec.data()), ldu, reinterpret_cast<MKL_Complex8*> (v_vec.data()), ldv);
        ...
        std::vector<double> s_vec (n,0);
        info = LAPACKE_zgesdd (LAPACK_ROW_MAJOR , 'S' , m, n , reinterpret_cast<MKL_Complex16*> (A_data) , lda, s_vec.data(), reinterpret_cast<MKL_Complex16*> (u_vec.data()), ldu, reinterpret_cast<MKL_Complex16*> (v_vec.data()), ldv);
